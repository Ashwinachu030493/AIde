...
class VectorStore:
    ...
    def _init_embedding_function(self):
        ...
        if explicit_path:
            if os.path.exists(explicit_path):
                ...
                logger.warning("EMBEDDING_MODEL_PATH is set but does not exist; falling back to model name.")
    ...
    def _add_code_snippet_sync(self, project_id: str, code: str, file_path: str, function_name: Optional[str] = None) -> str:
    ...
    async def add_code_snippet(self, project_id: str, code: str, file_path: str, function_name: Optional[str] = None) -> str:
        """Add code snippet async wrapper"""
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            None,
            lambda: self._add_code_snippet_sync(project_id, code, file_path, function_name)
        )

    def _query_similar_code_sync(self, project_id: str, query: str, n_results: int = 5) -> List[Dict]:
    ...
    async def query_similar_code(self, project_id: str, query: str, n_results: int = 5) -> List[Dict]:
        """Query similar code async wrapper - prevents event loop blocking"""
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            None,
            lambda: self._query_similar_code_sync(project_id, query, n_results)
        )
