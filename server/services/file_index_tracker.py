...
class FileIndexTracker:
    ...
    def update_file_status(
        ...
        record = self.db.query(FileIndexStatus).filter_by(
    ...
    def get_project_stats(self, project_id: str) -> Dict:
        ...
        total = self.db.query(func.count(FileIndexStatus.id)).filter_by(project_id=project_id).scalar() or 0
        indexed = self.db.query(func.count(FileIndexStatus.id)).filter_by(project_id=project_id, status='indexed').scalar() or 0
        errors = self.db.query(func.count(FileIndexStatus.id)).filter_by(project_id=project_id, status='error').scalar() or 0
        chunks = self.db.query(func.sum(FileIndexStatus.chunks_count)).filter_by(project_id=project_id).scalar() or 0
    ...
    def get_recently_indexed(self, project_id: str, limit: int = 10) -> List[Dict]:
        ...
        files = self.db.query(FileIndexStatus).filter_by(
            project_id=project_id,
            status='indexed'
        ).order_by(
            FileIndexStatus.indexed_at.desc()
        ).limit(limit).all()
