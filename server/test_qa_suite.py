...
class TestResults:
    ...
    def summary(self):
        ...
        print(f"{'='*60}")
        print(
            f"Total: {total} | {Colors.GREEN}Passed: {self.passed}{Colors.RESET} | {Colors.RED}Failed: {self.failed}{Colors.RESET}"
        )
        if self.errors:
...
def test_settings_api():
    ...
    try:
        resp = requests.post(
            f"{BASE_URL}/settings/preferences",
...
def test_ingestion_api():
    ...
    try:
        resp = requests.post(
            ...
            data={
                "project_path": "f:/AIde/server/audit",
                "project_id": "test_qa_project",
                "max_workers": "2",
            },
            timeout=10,
        )
        ...
        log_pass(
            ...
            f"(status: {status_data.get('status')}, processed: {status_data.get('processed_files')})",
        )
        results.add_pass()
...
def test_chat_api():
    ...
    print(
        f"{Colors.YELLOW}ℹ INFO{Colors.RESET} - Chat uses WebSocket at ws://localhost:8000/chat/ws"
    )
    print(
        f"{Colors.YELLOW}ℹ INFO{Colors.RESET} - Manual testing recommended for full chat verification"
    )
...
def test_edge_cases():
    ...
    try:
        resp = requests.post(f"{BASE_URL}/audit/file", json={}, timeout=5)  # Missing file_path
