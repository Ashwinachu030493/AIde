...
def test_file_ingestion():
    ...
    try:
        res = requests.post(f"{BASE_URL}/ingestion/file", data={
            "content": content, "file_path": "test.py", "project_id": "test_res"
        })
...
def test_project_ingestion():
    ...
    try:
        # Scan self
        res = requests.post(f"{BASE_URL}/ingestion/project", data={
            "project_path": os.getcwd(), "project_id": "self_scan", "max_workers": 2
        })
        if res.status_code == 200:
