...
class LLMClient:
    ...
    def __init__(self, user_config: Optional[UserLLMConfig] = None, db_session: Optional[Session] = None):
    ...
    async def get_completion(
        ...
        **kwargs,
        ...
        try:
                ...
                async def stream_gen():
                    ...
                    async for chunk in response:
                        if chunk.choices and chunk.choices[0].delta.content:
                            ...
                            yield content
                    await self._log_usage(provider, model_name, operation, prompt_tokens, full_response, project_id)
            ...
            await self._log_usage(provider, model_name, operation, prompt_tokens, content, project_id)
                        ...
                        async def fb_stream_gen():
                            ...
                            async for chunk in response:
                                if chunk.choices and chunk.choices[0].delta.content:
                                    ...
                                    full_response += content
                                    yield content
                            await self._log_usage(provider, fallback_model, f"{operation}_fallback", prompt_tokens, full_response, project_id)
                            self._update_stats(provider, full_response)
                        return fb_stream_gen()
                    else:
                        response = await acompletion(**params)
                        content = response.choices[0].message.content
                        await self._log_usage(provider, fallback_model, f"{operation}_fallback", prompt_tokens, content, project_id)
                        self._update_stats(provider, content)
                        return content
                except Exception as fb_err:
                    logger.error(f"Fallback model {fallback_model} also failed: {fb_err}")
            # Log failure
            await self._log_usage(provider, model_name, f"{operation}_failed", prompt_tokens, "", project_id, error=str(e))
            raise

    # Helpers -----------------------------------------------------------------
    def _resolve_model(self, model: Optional[str], task_type: str, user_tier: str) -> tuple[str, Optional[str]]:
        if model:
            return model, None
