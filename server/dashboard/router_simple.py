...
async def get_project_overview(project_id: str, db: Session = Depends(get_db)) -> JSONResponse:
...
async def _get_llm_usage_stats(project_id: str, db: Session) -> Dict:
    ...
    total_stats = (
        ...
        .filter(LLMUsageLog.project_id.contains(project_id))
        .first()
    )
...
async def _get_audit_stats(project_id: str, db: Session) -> Dict:
    ...
    latest_run = (
        ...
        .order_by(desc(AuditRun.completed_at))
        .first()
    )
