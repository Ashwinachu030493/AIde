...
from server.ingestion.parser import CodeParser, FileMetadata, CodeChunk
    ...
    try:
        ...
        for i in range(0, len(scan_results), batch_size):
            ...
            for meta, chunks, errs in batch:
                if not meta: continue # Failed scan
                for chunk in chunks:
...
async def ingest_file(
    ...
    try:
        loop = asyncio.get_event_loop()
        # Parse in thread
        metadata, chunks = await loop.run_in_executor(executor, parser.parse_file, file_path, content)
