...
class PersistentAuditor(AuditEngine):
    ...
    def audit_project_persistent(self, project_id: str, project_path: str) -> Dict[str, Any]:
        ...
        audit_run = AuditRun(
            project_id=project_id, status="processing", started_at=datetime.utcnow()
        )
        ...
        try:
            ...
            audit_run.high_issues = severity_counts.get(
                "high", 0
            )  # Mapping needed if enum distinct
    ...
    def _calculate_health_score(self, violations: List[AuditViolation], total_files: int) -> int:
        ...
        # Penalties per issue
        penalty_map = {Severity.CRITICAL: 10, Severity.WARNING: 3, Severity.INFO: 0.5}
