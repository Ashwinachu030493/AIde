...
def test_settings_llm_integration():
            ...
            try:
                async with websockets.connect(uri) as websocket:
                    ...
                    await websocket.send(
                        json.dumps({"type": "message", "content": "Hello", "context": {}})
                    )
    ...
    try:
        ...
        audit_res = requests.post(
            ...
            params={"project_path": test_dir, "project_id": "test_project"},
        )
            ...
            for _ in range(5):
                ...
                if runs_res.status_code == 200:
                    ...
                    if runs.get("total_runs", 0) > 0:
                        run = runs["runs"][0]
                        print(
                            f"   Audit run completed: Status={run['status']}, Issues={run['total_issues']}"
                        )
                        break
            else:
