...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('aide.start', () => {
...
async function checkServerConnection() {
    try {
        ...
        const item = await vscode.window.showWarningMessage('⚠️ AIde server not detected. Is it running?', 'Try Again');
        if (item === 'Try Again') {
            checkServerConnection();
        }
    }
...
ChatViewProvider.viewType = 'aide.chatView';
